...
mod tests {
    ...
    /// Fuzz `FastSha512` against `sha2::Sha512` over random lengths and
    ...
    ///
    /// Run with: `cargo test --release --lib --features fast-sha512 fuzz_against_sha2 -- --ignored --nocapture`
    #[test]
    #[ignore]
    fn fuzz_against_sha2() {
        ...
        for case in 0..CASES {
            ...
            // 2. FastSha512 with random chunk splits.
            {
                let mut h = FastSha512::new();
                let mut cursor = 0;
                while cursor < n {
